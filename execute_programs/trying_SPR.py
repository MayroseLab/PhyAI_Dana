...
def regraft_branch(t_cp_p, rgft_node, prune_node_cp, rgft_name, nname):
	...
	print("1:",nname)
	print("2:",rgft_name)
	print(t_curr.get_ascii(show_internal=True))
...
def create_SPR_job(dataset_path, step_number, rewrite_phy, runover):
	print("**************************************\n", dataset_path)
	job_name = "SPR_for_ds"
	cmd = "python " + CODE_PATH + "execute_programs/SPR_move.py -ds " + dataset_path + " -st " + str(step_number)
	if runover:
		cmd += " -r "
	if rewrite_phy:
		cmd += " -phy "
...
if __name__ == '__main__':
	...
	if dataset_path:
		if args.step_number == "1":
			...
			df_sum = pd.read_csv(SUMMARY_PER_DS.format(args.dataset_path, "prune", "br", prev_step)).set_index('Unnamed: 0')
			best_tree_id = df_sum["ll"].astype(float).idxmin()
		...
		with open("log_SPR.txt", 'w') as f:
			f.write("step: {}\nstart index: {}\nnumber of lines: {}".format(args.step_number, args.index_to_start_run, args.nline_to_run))
		csv_path = SUMMARY_FILES_DIR + CHOSEN_DATASETS_FILENAME
		res = traverse_data_dirs(create_SPR_job, csv_path, (args.index_to_start_run, args.nline_to_run), args.step_number, args.rewrite_in_phylip, args.runover)