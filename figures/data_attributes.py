...
def corr_plot(df):
	...
	plt.text(1.4, 1.2, "b$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.tight_layout()
	plt.show()
	...
	axes[0].text(-1.4, 1.5, "c$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.xlim(-0.3, 31)
	plt.ylim(0, 1)
	plt.tight_layout()
	plt.show()
	...
	#df["Database"] = df["Database"].replace(["ploiDB"], "PloiDB")
	#df["Database"] = df["Database"].replace(["protDBs"], "ProtDBs")
	#df["Database"] = df["Database"].replace(["selectome"], "Selectome")
	...
	plt.ylabel("")
	plt.setp(ax3.get_xticklabels(), rotation=20)
	plt.text(-0.6, 1.2, "a$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.tight_layout()
	plt.show()
...
def corr_plot_more_atts(df):
	...
	plt.legend(handles=[stats_patch])
	plt.text(-250, 1.2, "d$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.tight_layout()
	plt.show()

	ax1 = sns.jointplot(x="gaps", y=SCORES_LST[0], data=df, kind='reg', stat_func=pearsonr, line_kws={'color':'black'}, color=next(palette), xlim=(0,68), ylim=(0,1))#, ax=ax1)
	plt.xlabel('Average gaps (%)')
	#plt.ylabel('Spearman correlation ({})'.format(r'$\rho$'))
	plt.ylabel("")
	#stats_patch = mpatches.Patch(color='white', label='$r^2$ = 0.01;  $pval$ = $1.1x10^-$$^1$$^8$')#, contains=False)
	stats_patch = mpatches.Patch(color='white',label='$r^2$ = 0.11;  $pval$ = $1.3x10^-$$^2$$^8$')  # , contains=False)
	plt.legend(handles=[stats_patch])
	plt.text(-3.1, 1.2, "e$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.tight_layout()
	plt.show()
	...
	#next(palette)
	#df = df[df["dev_ult"] <= 15]
	ax2 = sns.jointplot(x="dev_ult", y=SCORES_LST[0], data=df, kind='reg', stat_func=pearsonr, line_kws={'color':'black'}, color=next(palette), xlim=(0,0.6), ylim=(0,1))#, ax=ax2)
	...
	stats_patch = mpatches.Patch(color='white', label='$r^2$ = 0.03;  $pval$ = $7.4x10^-$$^1$$^0$')  # , contains=False)
	plt.legend(handles=[stats_patch])
	plt.text(-0.033, 1.2, "f$_{2}$", fontsize=20, fontweight='bold', va='top', ha='right')
	plt.tight_layout()
	plt.show()
