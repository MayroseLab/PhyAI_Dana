...
def index_shared_features(dff, ind, edge, move_type, features_dicts_dict):
    ...
    for subtype in ["p", "r"]:
        dff.loc[ind, FEATURES["ntaxa_{}".format(subtype)]] = features_dicts_dict["ntaxa_{}".format(subtype)][
            edge]  # f4,5
        # dff.ix[ind, FEATURES["pdist_{}".format(subtype)]] = features_dicts_dict["pdist_{}".format(subtype)][edge]    	#f6,7
        dff.loc[ind, FEATURES["tbl_{}".format(subtype)]] = features_dicts_dict["tbl_{}".format(subtype)][edge]  # f8,9
        # dff.ix[ind, FEATURES["pars_{}".format(subtype)]] = features_dicts_dict["pars_{}".format(subtype)][edge] 		#f10,11
        dff.loc[ind, FEATURES["longest_{}".format(subtype)]] = features_dicts_dict["longest_{}".format(subtype)][
            edge]  # f12,13
...
def collect_features(ds_path, step_number, outpath_prune, outpath_rgft, tree_type='bionj'):
    ...
    for i, row in dfr.iterrows():
        ...
        # pass
        if not "subtree" in row["rgft_name"] and not ROOTLIKE_NAME in row["rgft_name"] and not ROOTLIKE_NAME in row[
            "prune_name"]:
            ...
            df_rgft = index_additional_rgft_features(df_rgft, ind, row["prune_name"], row["rgft_name"],
                                                     features_restree_dict,
                                                     features_prune_dicts_dict)  # also prune dict because for 2 features i didn't want to comp dict within each rgft iteration (needed to compute on the starting tree)
        ...
        for j, rgft_name in enumerate(os.listdir(prune_dirpath)):
            ...
            if not "subtree" in rgft_name:  # subtrees are dealt separately ~10 lines above
                if cp_internal:
                    ...
                    rearr_tree_path = SEP.join(
                        [tree_dirpath, "{}_phyml_{}_{}.txt".format(MSA_PHYLIP_FILENAME, "tree", OPT_TYPE)])
...
def create_parsing_job(dataset_path, cp_internal, step_number, tree_type):
    ...
    cmd = "python " + CODE_PATH + "summary/collect_SPR_features.py -ds " + dataset_path + " -st " + str(
        step_number) + " -ttype " + tree_type
    if cp_internal:
        cmd += " -cp "
...
if __name__ == '__main__':
    ...
    if dataset_path:
        ...
        traverse_data_dirs(create_parsing_job, csv_path, (args.index_to_start_run, args.nline_to_run), args.cp_internal,
                           args.step_number, args.tree_type)