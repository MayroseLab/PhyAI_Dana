...
DIRPATH = '/groups/itay_mayrose/danaazouri/'
...
df_train = pd.read_csv(DIRPATH + r'train_data/train_task_1_2.csv')
...
complete_df.to_csv(DIRPATH + 'merged_data.csv')
...
'''
'''
complete_df = pd.read_csv(DIRPATH + 'merged_data.csv')
...
label = special_cols[-1]
pivot_cols = special_cols[:-1]
complete_df_encoded = pd.read_csv(DIRPATH + 'merged_data_encoded.csv')
features = remove_from_lst(list(complete_df_encoded), special_cols)
...
df_transformed_X = complete_df_encoded.head(100000).pivot(
    index=pivot_cols[1],
    columns=pivot_cols[0],
    values=features).fillna(0)

df_transformed_Y = complete_df_encoded.head(100000).pivot(
    index=pivot_cols[1],
    columns=pivot_cols[0],
...
########## additional attempts #########
df_transformed_X = complete_df_encoded.head(1000).groupby([pivot_cols[1], pivot_cols[0]])[features].max().unstack()
df_transformed_Y = complete_df_encoded.head(1000).groupby([pivot_cols[1], pivot_cols[0]])[label].max().unstack()
...
model_knn = KNeighborsClassifier(metric='cosine', n_neighbors=5, weights='distance', algorithm='auto', n_jobs=-1)   #metric='cosine'
...
print("Accuracy on validation set:",metrics.accuracy_score(y_val, y_pred))