...
class DynaAgent:
    ...
    def __init__(self, exp_rate=0.3, lr=0.1, n_steps=5, episodes=1):
        '''
        ## An addition for Dyna-Q+ algo:
        self.time = 0                 # keep track of the total time
        self.timeWeight = timeWeight  # the kappa parameter
        '''
        self.maze = Maze()
        self.state = S
        ...
        for ep in range(self.episodes):
            while not self.maze.end:
                ...
                for a in self.actions:
                    ...
                    if a != action:
                        self.model[self.state][a] = (0, self.state, 1)

                self.model[self.state][action] = (reward, nxtState, self.time)
                '''
                ...
                for _ in range(self.steps):
                    ...
                    '''
                    ## An addition for Dyna-Q+ algo:
                    _reward += self.timeWeight * np.sqrt(self.time - _time)   # the number of times that not been visited
                    '''
